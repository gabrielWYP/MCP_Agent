...
Used only by `fusion_mode="cross_attention"` (`MasterModel`'s restored
two-stream path). The default `fusion_mode="early"` has no cross-modal
attention at all — RGB and NIR are stacked before a single stem — so nothing
in this file is constructed or contributes a state_dict key under that mode.
Restored verbatim from the pre-redesign architecture so the two-stream
checkpoints written before it (`checkpoints/mastermodel_mango`) still load.
